...
class Config:
    ...
    NUM_EPOCHS = 200
    BASE_SEED = 42 # ベースシード
    MODEL_SAVE_DIR = 'models/models_2d' # モデル保存ディレクトリ
    LOG_DIR = 'log/logs_2d' # ログ保存ディレクトリ
    ANALYSIS_DIR = 'analysis/analysis_2d' # 分析結果保存ディレクトリ
...
def run_experiment(experiment_id, config):
    seed_everything(config.BASE_SEED + experiment_id) # 実験ごとにシードを変更
    ...
    if best_model_state_dict:
        ...
        torch.save(model.state_dict(), model_save_path) # Save last state if no improvement
    ...
    # この実験での最高のTrain Token AccとVal Seq Accを返す
    # best_epoch_val_seq_accuracy はこの実験でのEarly Stopping基準の最良値
    # current_best_train_token_acc はこの実験の全エポック中の最良値
...
if __name__ == '__main__':
    ...
    print("\n--- Inference Example (using model from last experiment) ---")
    # 最後の実験のモデルで推論例を実行 (必要に応じて変更)
    # この部分は、最良モデルを別途保存・ロードするロジックを追加することも可能
    
    # 最後の実験のモデルパスを構築
    last_exp_model_path = os.path.join(Config.MODEL_SAVE_DIR, f'model_exp{Config.NUM_EXPERIMENTS - 1}.pth')
    if os.path.exists(last_exp_model_path):
        # 語彙の再構築が必要 (run_experiment内で行っているため、ここでは簡易的に最後の実験のものを流用する想定)
        # 実際には、語彙を保存・ロードするか、推論専用の関数で再度構築する
        train_val_data_pairs_for_vocab = load_csv_data(Config.DATA_PATH)
        test_data_pairs_for_vocab = load_csv_data(Config.TEST_DATA_PATH)
        token_to_id_infer, id_to_token_infer = build_vocab(
            [train_val_data_pairs_for_vocab, test_data_pairs_for_vocab] if test_data_pairs_for_vocab else [train_val_data_pairs_for_vocab],
            Config.MAX_DIGITS
        )
        vocab_size_infer = len(token_to_id_infer)
        PAD_ID_infer = token_to_id_infer[PAD_TOKEN]
        SOS_ID_infer = token_to_id_infer[SOS_TOKEN]
        EOS_ID_infer = token_to_id_infer[EOS_TOKEN]

        model_infer = Seq2SeqTransformer(input_vocab_size=vocab_size_infer,
                                   output_vocab_size=vocab_size_infer,
                                   d_model=Config.D_MODEL,
                                   nhead=Config.NHEAD,
                                   num_encoder_layers=Config.NUM_ENCODER_LAYERS,
                                   num_decoder_layers=Config.NUM_DECODER_LAYERS,
                                   ...
                                   dropout_p=Config.DROPOUT_P,
                                   max_seq_len=Config.MAX_LEN).to(DEVICE)
        model_infer.load_state_dict(torch.load(last_exp_model_path, map_location=DEVICE))
        model_infer.eval()

        with torch.no_grad():
            test_eq = "25+30="
            src_token_ids = tokenize_and_numericalize(test_eq, token_to_id_infer, Config.MAX_DIGITS)
            src_tensor = torch.tensor(src_token_ids, dtype=torch.long).unsqueeze(1).to(DEVICE)
            src_padding_mask_infer = (src_tensor == PAD_ID_infer).transpose(0,1).to(DEVICE)

            memory_infer = model_infer.encode(src_tensor, src_mask=None, src_padding_mask=src_padding_mask_infer)
            current_memory_key_padding_mask_infer = src_padding_mask_infer[0:1, :] 

            generated_ids_infer = [SOS_ID_infer]
            ys_infer = torch.ones(1, 1).fill_(SOS_ID_infer).type_as(src_tensor.data).to(DEVICE)
            ...
            for _ in range(Config.MAX_LEN -1): 
                ...
                decoder_output_infer = model_infer.decode(ys_infer, memory_infer, 
                                                    tgt_causal_mask_infer,
                                                    current_tgt_padding_mask_infer, 
                                                    current_memory_key_padding_mask_infer) 
                
                last_token_logits_infer = model_infer.generator(decoder_output_infer[-1, :, :])
                next_token_id_infer = last_token_logits_infer.argmax(dim=-1).item()
                generated_ids_infer.append(next_token_id_infer)

                if next_token_id_infer == EOS_ID_infer:
                    break
            ...
            result_str = ids_to_string(generated_ids_infer, id_to_token_infer, SOS_ID_infer, EOS_ID_infer, PAD_ID_infer)
            
            print(f"Input: {test_eq}")
            print(f"Predicted (using model from exp {Config.NUM_EXPERIMENTS}): {result_str}")
    else:
        print(f"Could not find model from last experiment ({last_exp_model_path}) for inference example.")
