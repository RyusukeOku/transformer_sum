...
def generate_unique_addition_samples(num_samples: int, existing_samples: set = None):
    ...
    while len(new_samples) < num_samples and attempts < max_attempts:
        ...
        # --- 形式1: (num1_decomposed)+(num2_decomposed)= ---
        expression1 = f"({num1_tens}+{num1_units})+({num2_tens}+{num2_units})="
        ...
        # --- 形式2: (num2_decomposed)+(num1_decomposed)= (num1 と num2 が異なる場合) ---
        if num1 != num2 and len(new_samples) < num_samples:
            # num1とnum2が入れ替わるので、分解された表現も入れ替わる
            # (num2_tens+num2_units) + (num1_tens+num1_units) =
            expression2 = f"({num2_tens}+{num2_units})+({num1_tens}+{num1_units})="
            sample_pair2 = (expression2, answer) # 答えは同じ
    ...
    if len(new_samples) < num_samples:
        print(f"Warning: Could only generate {len(new_samples)} unique samples out of the requested {num_samples}. "
              ...
              f"total number of unique possible combinations (~8100 for 2-digit+2-digit, considering A+B and B+A as distinct decomposed forms if A!=B), "
...
if __name__ == "__main__":
    ...
    train_filename = "data/addition_train_decomposed_commutative_dataset.csv"
    test_filename = "data/addition_test_decomposed_commutative_dataset.csv"
